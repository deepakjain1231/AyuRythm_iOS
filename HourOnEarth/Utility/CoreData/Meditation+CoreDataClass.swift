...
public class Meditation: NSManagedObject {
    class func createMeditationData(dicData: [String: Any], needToSave:Bool = true) -> Meditation? {
        ...
        guard let entity = CoreDataHelper.sharedInstance.createEntityWithName("Meditation", uniqueKey: "id", value: id) as? Meditation else {
            return nil
        }
        entity.id = Int(id) ?? 0
...
//        }
    ...
    class func updateMeditationForMyList(id: String, selectedList: String) {
        guard let entity = CoreDataHelper.sharedInstance.createEntityWithName("Meditation", uniqueKey: "id", value: id) as? Meditation else {
            return
        }
