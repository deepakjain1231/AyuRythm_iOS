...
extension Keychain {
    static var bundleIdentifier: String {
        return Bundle.main.bundleIdentifier ?? "com.hoe.houronearth"
    }
...
extension Keychain {
    ...
    static var appCurrentVersion: String? {
        get {
            return try? Keychain(service: bundleIdentifier, account: "appCurrentVersion").readItem()
        }
        set {
            guard let value = newValue else {
                try? Keychain(service: bundleIdentifier, account: "appCurrentVersion").deleteItem()
                return
            }
            do {
                try Keychain(service: bundleIdentifier, account: "appCurrentVersion").saveItem(value)
            } catch {
                print("Unable to save appCurrentVersion to keychain.")
            }
        }
    }
}
