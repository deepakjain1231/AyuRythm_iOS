...
#import "TextTagCollectionView.h"

#pragma mark - -----TextTagConfig-----

@implementation TextTagConfig
...
- (instancetype)copyWithZone:(NSZone *)zone {
    TextTagConfig *newConfig = [TextTagConfig new];
...
@interface TextTagLabel : UIView
@property (nonatomic, strong) TextTagConfig *config;
@property (nonatomic, strong) GradientLabel *label;
@property (nonatomic, strong) CAShapeLayer *borderLayer;
@property (assign, nonatomic) BOOL selected;
@end

@implementation TextTagLabel
...
#pragma mark - -----TextTagCollectionView-----

@interface TextTagCollectionView () <TagCollectionViewDataSource, TagCollectionViewDelegate>
@property (strong, nonatomic) NSMutableArray <TextTagLabel *> *tagLabels;
@property (strong, nonatomic) TagCollectionView *tagCollectionView;
@end

@implementation TextTagCollectionView
...
- (void)commonInit {
    ...
    _defaultConfig = [TextTagConfig new];

    _tagCollectionView = [[TagCollectionView alloc] initWithFrame:self.bounds];
...
- (void)addTag:(NSString *)tag withConfig:(TextTagConfig *)config {
...
- (void)addTags:(NSArray<NSString *> *)tags withConfig:(TextTagConfig *)config {
...
- (void)insertTag:(NSString *)tag atIndex:(NSUInteger)index withConfig:(TextTagConfig *)config {
    if ([tag isKindOfClass:[NSString class]]) {
...
- (void)insertTags:(NSArray<NSString *> *)tags atIndex:(NSUInteger)index withConfig:(TextTagConfig *)config {
    [self insertTags:tags atIndex:index withConfig:config copyConfig:YES];
}

- (void)insertTags:(NSArray<NSString *> *)tags atIndex:(NSUInteger)index withConfig:(TextTagConfig *)config copyConfig:(BOOL)copyConfig {
    if (![tags isKindOfClass:[NSArray class]] || index > _tagLabels.count || ![config isKindOfClass:[TextTagConfig class]]) {
        return;
    }
    ...
    for (NSString *tagText in tags) {
        TextTagLabel *label = [self newLabelForTagText:[tagText description] withConfig:config];
...
- (void)removeTag:(NSString *)tag {
    ...
    for (TextTagLabel *label in _tagLabels) {
...
- (void)setTagAtIndex:(NSUInteger)index withConfig:(TextTagConfig *)config {
    if (index >= _tagLabels.count || ![config isKindOfClass:[TextTagConfig class]]) {
        return;
    }
...
- (void)setTagsInRange:(NSRange)range withConfig:(TextTagConfig *)config {
    if (NSMaxRange(range) > _tagLabels.count || ![config isKindOfClass:[TextTagConfig class]]) {
        return;
    }
    
    NSArray *tagLabels = [_tagLabels subarrayWithRange:range];
    config = [config copy];
    for (TextTagLabel *label in tagLabels) {
        label.config = config;
    }
    [self reload];
}
...
- (NSArray<NSString *> *)getTagsInRange:(NSRange)range {
    if (NSMaxRange(range) <= _tagLabels.count) {
        ...
        for (TextTagLabel *label in [_tagLabels subarrayWithRange:range]) {
...
- (TextTagConfig *)getConfigAtIndex:(NSUInteger)index {
    if (index < _tagLabels.count) {
...
- (NSArray<TextTagConfig *> *)getConfigsInRange:(NSRange)range {
    if (NSMaxRange(range) <= _tagLabels.count) {
...
- (NSInteger)indexOfTagAt:(CGPoint)point {
    // We expect the point to be a point wrt to the TextTagCollectionView.
    // so convert this point first to a point wrt to the TagCollectionView.
...
- (BOOL)tagCollectionView:(TagCollectionView *)tagCollectionView shouldSelectTag:(UIView *)tagView atIndex:(NSUInteger)index {
    if (_enableTagSelection) {
        TextTagLabel *label = _tagLabels[index];
...
- (void)updateStyleAndFrameForLabel:(TextTagLabel *)label {
...
- (TextTagLabel *)newLabelForTagText:(NSString *)tagText withConfig:(TextTagConfig *)config {
    TextTagLabel *label = [TextTagLabel new];
