//
//  FavouriteFood+CoreDataClass.swift
//  HourOnEarth
//
...
@objc(FavouriteFood)
public class FavouriteFood: NSManagedObject {
    class func createFoodData(dicData: [String: Any]) -> FavouriteFood? {
        let id = dicData["id"] as? String ?? ""
        guard let entity = CoreDataHelper.sharedInstance.createEntityWithName("FavouriteFood", uniqueKey: "id", value: id) as? FavouriteFood else {
            return nil
        }
        entity.id = Int(id)  ?? 0
...
//        if let seasons = dicData["season"] as? [[String: Any]]
//        {
