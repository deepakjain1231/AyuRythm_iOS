...
@objc(Pranayama)
public class Pranayama: NSManagedObject {
    
    class func createPranayamaData(dicData: [String: Any], needToSave:Bool = true) -> Pranayama? {
        ...
        guard let entity = CoreDataHelper.sharedInstance.createEntityWithName("Pranayama", uniqueKey: "id", value: id) as? Pranayama else {
            return nil
        }
        entity.id = Int(id) ?? 0
        ...
        entity.descriptionPranayama = dicData["description"] as? String ?? ""
        entity.star = dicData["star"] as? String ?? "no"
        entity.types_flag = dicData["types_flag"] as? String ?? ""
        ...
        entity.file_type = dicData["file_type"] as? String ?? ""
...
//        }
    ...
    class func updatePranayamaForMyList(id: String, selectedList: String) {
        guard let entity = CoreDataHelper.sharedInstance.createEntityWithName("Pranayama", uniqueKey: "id", value: id) as? Pranayama else {
            return
        }
