...
@objc(Kriya)
public class Kriya: NSManagedObject {
    class func createKriyaData(dicData: [String: Any], needToSave:Bool = true) -> Kriya? {
        ...
        guard let entity = CoreDataHelper.sharedInstance.createEntityWithName("Kriya", uniqueKey: "id", value: id) as? Kriya else {
            return nil
        }
        entity.id = Int(id) ?? 0
...
//        }
    ...
    class func updateKriyaForMyList(id: String, selectedList: String) {
        guard let entity = CoreDataHelper.sharedInstance.createEntityWithName("Kriya", uniqueKey: "id", value: id) as? Kriya else {
            return
        }
