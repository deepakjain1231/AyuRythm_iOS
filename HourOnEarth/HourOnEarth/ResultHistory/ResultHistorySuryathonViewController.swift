//
//  ResultHistorySuryathonViewController.swift
//  HourOnEarth
//
//  Created by Paresh Dafda on 15/07/21.
...
let serverSendDateFormat = "yyyy-MM-dd"

class ResultHistorySuryathonViewController: BaseViewController, ChartViewDelegate {
...
extension ResultHistorySuryathonViewController {
    func getHistoryDataFromServer(params: [String : Any], completion: @escaping (Bool, String, String, [ARChartData])->Void) {
        if Utils.isConnectedToNetwork() {
            ...
            let urlString = kBaseNewURL + endPoint.getSuryathonTrackerData.rawValue
            print(">>>> PARAMS :: ", params)
            dataFetchRequest = AF.request(urlString, method: .post, parameters: params, encoding:URLEncoding.default, headers: headers).validate().responseJSON(queue: DispatchQueue.main, options: JSONSerialization.ReadingOptions.allowFragments)  { response in
                ...
                case .success(let value):
                    ...
                    if let data = dicResponse["data"] as? [[String: String]] {
                        data.forEach{
                            if let total = $0["total"], let createdAt = $0["created_at"] {
                                let item = ARChartData(total: total, createdAt: createdAt)
...
struct ARChartData {
    ...
    func displayDateString(for filter: HistoryResultFilter) -> String {
        switch filter {
        case .day:
            return createdAt.dateString(fromFormat: "HH:mm:ss", toFormat: "hh:mm a")
        
        case .week:
            return createdAt.dateString(fromFormat: serverSendDateFormat, toFormat: "dd/MM")
            
        case .month:
            return createdAt.dateString(fromFormat: serverSendDateFormat, toFormat: "dd")
